...
### PROP-2026-08-12 · story/code collision — narrative framing routed to The Engineer

- **date:** 2026-08-12
- **signal:** manual routing probe — "Write a story about a programmer debugging code at 3 AM.
  Make it funny but also technically accurate about async/await patterns." routed to `coding-hinge`
  (The Engineer)
- **observed failure:** a clear story request with incidental coding vocabulary (`async`, `await`)
  took the coding route and persona instead of `story-architect`
- **root cause:** `coding-hinge` matchTerms include `async`/`await`; the coding lane runs before the
  story lane and catalog order also places `coding-hinge` before `story-architect`, so any
  narrative mentioning code loses to the coding terms
- **proposed change:** `hasNarrativeFraming(text)` gate on the coding lane — a creation verb
  (`write`/`tell`/`build`/`create`/...) directly governing a story noun (`story`/`tale`/`narrative`)
  suppresses the coding lane, mirroring the existing `hasComparisonFraming` pattern. Deliberately
  fires only on creation-verb → story-noun, so incidental story nouns in coding requests
  ("write a react component that tells a story") still route to coding
- **category:** `behavior`
- **status:** `applied`
- **verification:** the report's input → `story-architect`; "Explain async/await patterns in
