...
|-------|-------------------|-------------------------------|
| Current savings vs gpt-4o baseline (ceiling-based) | `npm test -- --runInBand --testPathPatterns=routingEval` | **89%** (routingEval 57), **87.4%** (ML implemented-route holdout), **87%** (blind implemented-route holdout), and **90%** (foreign holdout), measured 2026-09-02. Synthetic, corpus-specific, and pre-send modeled; not realized customer savings. |
...
|-------|-------------------|----------|
| 1366 tests / 121 suites | `npm test -- --runInBand` | ✅ 121/121 suites and 1366/1366 tests passed locally on 2026-09-02; hosted GitHub CI did not start because of an account-level billing lock. |
...
| Build succeeds | `npm run build` | ✅ (2,321 modules transformed in 6.45s on 2026-09-02) |
...
| Pooled accuracy in claimed 60–80% range | `npm test -- --runInBand src/__eval__/claimsSync.test.js` | ✅ 70.6% (96/136 unique samples), measured 2026-09-02 |
...
*Ledger re-audited 2026-09-02 against commit `4e729c2`. Re-run producing commands before any later public claim.*
