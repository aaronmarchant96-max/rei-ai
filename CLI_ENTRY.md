...
4. **ASK BEFORE TOUCHING CONFIGS:** Deployments are triggered automatically via Vercel webhooks (verified by the manual `verify-vercel-webhook` job in `.gitlab-ci.yml`). If deployments fail, **ASK THE USER**—never attempt to restructure the pipeline.
